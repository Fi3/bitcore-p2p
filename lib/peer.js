...
Peer.prototype._addSocketEventHandlers = function() {
  ...
  this.socket.on('data', function(data) {
    ...
    //}
    //catch(err) {
    //  if(err.message.indexOf(errorFilter) != -1 || err.message.indexOf(errorCommand) != -1) {
    //    var messageReject = new Messages().Reject();
    //    self.sendMessage(messageReject);
    //  }
    //  else {
    //    console.log('canne!!!');
    //  };
...
Peer.prototype._readMessage = function() {
  var errorFilter = 'TypeError: Data object should include number of hash ' +
    'functions "nHashFuncs" TypeError:';
  var errorFilter1 = 'Data object should include';
  var errorCommand = 'Unsupported message command';
  try {
    var message = this.messages.parseBuffer(this.dataBuffer);
  }
  catch(err) {
    if(err.message.toString().indexOf(errorFilter1) != -1 || err.message.
        toString().indexOf(errorCommand) != -1) {
      var messageReject = new Messages().Reject();
      this.sendMessage(messageReject);
    }
    else {
     throw err; 
    };
